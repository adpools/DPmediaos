...
import { Search, MoreHorizontal, Sparkles, Key, Loader2, Plus } from "lucide-react";
    ...
    <div className="space-y-10">
      ...
      <div className="flex gap-6 overflow-x-auto pb-4 scrollbar-hide">
        {isProjectsLoading ? (
          ...
          <>
            {projects?.map((proj) => (
              <Card key={proj.id} className={`min-w-[320px] h-[180px] border-none shadow-lg text-white rounded-[2rem] p-8 flex flex-col justify-between group cursor-pointer hover:scale-[1.02] transition-transform ${proj.color || 'card-pink'}`}>
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <h2 className="text-2xl font-bold font-headline">Upcoming Tasks</h2>
          <div className="flex gap-3">
             <Button variant="default" size="sm" className="bg-primary rounded-xl px-4 text-xs font-bold h-9 shadow-lg shadow-primary/20">+ New Task</Button>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between border-b border-white/40 pb-1">
          <Tabs defaultValue="active" className="w-auto">
            <TabsList className="bg-transparent h-auto p-0 gap-8">
              <TabsTrigger value="active" className="rounded-none border-b-2 border-transparent data-[state=active]:border-primary data-[state=active]:bg-transparent data-[state=active]:shadow-none text-xs font-bold px-0 pb-3">Active</TabsTrigger>
              <TabsTrigger value="completed" className="rounded-none border-b-2 border-transparent data-[state=active]:border-primary data-[state=active]:bg-transparent data-[state=active]:shadow-none text-xs font-bold px-0 pb-3 text-muted-foreground/60">Done</TabsTrigger>
            </TabsList>
          </Tabs>
        </div>
        ...
        <div className="space-y-4 pt-4">
          {isTasksLoading ? (
            ...
            <div className="text-center py-12 text-muted-foreground">
              <p className="text-sm">No tasks assigned for today. Time to plan!</p>
            </div>
          ) : (
            tasks?.map(task => (
              <div key={task.id} className="flex items-center justify-between group cursor-pointer p-4 hover:bg-white rounded-2xl transition-all shadow-sm">
                <div className="flex items-center gap-4">
                  ...
                  <div className="space-y-1">
                    ...
                    <p className="text-[11px] text-muted-foreground font-medium">{task.status}</p>
                  </div>
                </div>
